...
section-align <target>
   ...
   │      → wait for Codex/Codex bot review; sub-loop until clean
   │
...
Phases are sequential. Within a phase, dispatch subagents in parallel where independent — **hard cap of 3 parallel subagents** per `~/.Codex-shared/shared/AGENTS.md`.
...
|------|---------|--------|-----------------------|
| **PR mode** | `PR <n>` | check out PR head branch in worktree | yes — fetch and address prior Codex/Codex/human comments |
...
List every unresolved Codex/Codex/human comment, with the inline-comment ID for thread-replying in Phase 2. Also check `nobodies-collective/Humans` if the PR is cross-fork (`feedback_pr_review_both_repos`).
...
Push to the working branch. Wait for fresh Codex/Codex bot reviews. Address findings:
...
- **Resolve every prior review thread** (Codex, Codex, human). Fix if it makes sense; if it doesn't, reply explaining why. Per `feedback_codex_thread_replies`, thread-reply each finding directly via `gh api repos/.../pulls/<n>/comments/<id>/replies`, not as a top-level PR comment.
...
**"Done" means bot-review-clean.** Pushed + green CI ≠ done (`feedback_done_means_codex_clean`). Wait for fresh Codex/Codex review on each push and address findings before declaring a phase complete.
...
**Each push sub-loop**:
1. Push.
2. Wait for bot reviews to land (Codex + Codex both review on push for this repo).
...
**Concurrency**: max 3 parallel subagents per `~/.Codex-shared/shared/AGENTS.md`. Within a phase, dispatch independent work in parallel up to that cap; queue the rest.
