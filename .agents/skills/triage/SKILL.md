...
Requires env vars (set in the Humans project's `.Codex/settings.local.json`):
...
The issue quality matters — it's the handoff to the fixing phase. A good issue lets someone (or Codex) pick it up and start coding immediately without re-investigating.
